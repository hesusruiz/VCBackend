...
const (
	...
	// MulticodecKindECDSAPubKey p256-pub
	MulticodecKindECDSAPubKey = 0x1200
)
...
func NewID(pub crypto.PubKey) (ID, error) {
	switch pub.Type() {
	case crypto.Ed25519, crypto.RSA, crypto.ECDSA:
...
func (id ID) MulticodecType() uint64 {
	...
	case crypto.ECDSA:
		return MulticodecKindECDSAPubKey
	default:
...
func Parse(keystr string) (ID, error) {
	...
	case MulticodecKindEd25519PubKey:
		...
		return ID{pub}, nil
	case MulticodecKindECDSAPubKey:
		pub, err := crypto.UnmarshalECDSAPublicKey(data[n:])
		if err != nil {
			return id, err
		}
		return ID{pub}, nil
