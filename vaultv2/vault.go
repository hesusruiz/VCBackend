...
func (v *Vault) CreateOrGetUserWithDID(
	userid string,
	name string,
	usertype string,
	password string,
	didMethod string) (usr *ent.User, did string, err error) {

	// Check that we support the did method
	...
	zlog.Info().Str("DID", did).Str("id", userid).Str("name", name).Str("type", usertype).Msg("user created")
