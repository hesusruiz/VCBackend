...
func (uq *UserQuery) QueryKeys() *PrivateKeyQuery {
	query := &PrivateKeyQuery{config: uq.config}
...
func (uq *UserQuery) QueryDids() *DIDQuery {
	query := &DIDQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryCredentials() *CredentialQuery {
	query := &CredentialQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithDids tells the query-builder to eager-load the nodes that are connected to
// the "dids" edge. The optional arguments are used to configure the query builder of the edge.
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withKeys; query != nil {
		...
		query.Where(predicate.PrivateKey(func(s *sql.Selector) {
			s.Where(sql.InValues(user.KeysColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_keys
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_keys" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_keys" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Keys = append(node.Edges.Keys, n)
		}
	}

	if query := uq.withDids; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[string]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Dids = []*DID{}
		}
		query.withFKs = true
		query.Where(predicate.DID(func(s *sql.Selector) {
			s.Where(sql.InValues(user.DidsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_dids
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_dids" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_dids" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Dids = append(node.Edges.Dids, n)
		}
	}
	...
	if query := uq.withCredentials; query != nil {
		...
		query.Where(predicate.Credential(func(s *sql.Selector) {
			s.Where(sql.InValues(user.CredentialsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_credentials" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
