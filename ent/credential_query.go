...
func (cq *CredentialQuery) GroupBy(field string, fields ...string) *CredentialGroupBy {
	grbuild := &CredentialGroupBy{config: cq.config}
...
func (cq *CredentialQuery) Select(fields ...string) *CredentialSelect {
	...
	selbuild := &CredentialSelect{CredentialQuery: cq}
	selbuild.label = credential.Label
...
func (cq *CredentialQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Credential, error) {
	...
	if query := cq.withAccount; query != nil {
		...
		nodeids := make(map[string][]*Credential)
		for i := range nodes {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_credentials" returned %v`, n.ID)
			}
			for i := range nodes {
